...
class weuiModel {
	...
	//显示ActionSheet
	showActionSheet(labels, callback) {
		...
		//addeventListener
		this.addTapEvent(btncancel[0], function(element) {
			this.hideActionSheet();
			if(callback != null) {
				callback(-1);
			}
		});
		this.addTapEvent(mask[0], function(element) {
			this.hideActionSheet();
			if(callback != null) {
				callback(-1);
			}
		...
		list.children().each(function() {
			this.addTapEvent(this, function(element) {
				this.hideActionSheet();
				if(callback != null) {
	...
	//显示弹窗
	showAlert(title, content, confirmLabel, cancelLabel, callback) {
		...
		if(btnOK) {
			this.addTapEvent(btnOK[0], function(element) {
				if(callback != null) {
					callback(1);
				}
				div.remove();
